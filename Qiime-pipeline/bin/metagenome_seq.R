...
raw_counts<-read.delim(arg[1],sep="\t", skip = 1, header = TRUE)
counts<- raw_counts[,-1]

pdata = loadPhenoData(arg[2])
rownames(pdata)=gsub("-",".",rownames(pdata))
ord = match(colnames(counts), rownames(pdata)) 
pdata = pdata[ord, ]
phenotypeData = AnnotatedDataFrame(pdata)

taxa = as.data.frame(raw_counts[,1])
colnames(taxa)="OTU"
OTUdata = AnnotatedDataFrame(taxa)
data = newMRexperiment(counts,phenoData=phenotypeData,featureData = OTUdata)
...
# MAKE CONTRAST
contrastconds<-read.delim(file=arg[3], col.names=NA)
...
for (c in 1:length(rownames(contrastconds))){
  ...
  fit = eBayes(fit)
  top<-topTable(fit, number = nrow(counts))
